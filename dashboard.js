...
// 60 points = 30 minutes @ 30 s resolution
const HIST    = 60;
// Map trail unchanged
const TRAIL   = 50;
...
function initCharts() {
  ...
  SENSORS.forEach(s=>{
    ...
    s.chart = new Chart(ctx, {
      type: 'line',
      data: { datasets: [{ data: [], borderColor: s.col, borderWidth:2, tension:0.25 }] },
      options: {
        animation: false,
        responsive: true,
...
async function poll(){
  ...
  SENSORS.forEach(s=>{
    const val = live[s.id];
    if (val == null) return;
    s.chart.data.datasets[0].data.push({
      x: new Date(live.ts),
      y: val
    });
