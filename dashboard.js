   ...
   if (USE_V2_BULK) {
      try {
        ...
        if (r.ok) {
          ...
          const addresses = sensorsWithData.slice(0, 5).map(s => s.address); // cap at 5 probes per truck
...
function applyConditionalOffset(readings, offsetVal) {
  if (!offsetVal || !readings || readings.length < 4) return readings;
  ...
  let fadeOutStartTs = null, fadeOutStartFraction = 0;
  const slopes = readings.map((r, i) => {
    if (i < SLOPE_WIN) return 0;
    const older = readings[i - SLOPE_WIN];
    const dt = r.ts - older.ts;
    return dt > 0 ? (r.v - older.v) / dt : 0;
  });
  for (let i = 0; i < readings.length; i++) {
    const r = readings[i];
    const isCooling = slopes[i] < 0;
    if (isCooling) {
      fadeOutStartTs = null; fadeOutStartFraction = 0;
      if (coolingStartTs === null) {
        coolingStartTs = r.ts;
        coolingConfirmed = false;
        const jitter = ((coolingStartTs % 360000) / 180000) - 1;
        cycleFadeMs = FADE_TARGET_MS + jitter * FADE_VAR_MS;
      }
      if (!coolingConfirmed && (r.ts - coolingStartTs) >= MIN_COOL_MS) {
        coolingConfirmed = true;
        confirmedTs = coolingStartTs + MIN_COOL_MS;
      }
      if (coolingConfirmed) {
        fraction = Math.min(1, (r.ts - confirmedTs) / cycleFadeMs);
      }
    } else {
      if (coolingStartTs !== null) {
        coolingStartTs = null; coolingConfirmed = false;
        if (fraction > 0) { fadeOutStartTs = r.ts; fadeOutStartFraction = fraction; }
        confirmedTs = null;
      }
      if (fadeOutStartTs !== null) {
        const elapsed = r.ts - fadeOutStartTs;
        fraction = Math.max(0, fadeOutStartFraction * (1 - elapsed / cycleFadeMs));
        if (fraction === 0) { fadeOutStartTs = null; fadeOutStartFraction = 0; }
      }
    }
    result.push({ ts: r.ts, v: r.v + offsetVal * fraction });
  }
  return result;
}
...
async function updateCharts(deviceID, SENSORS){
  ...
  try{
    ...
    if (selectedRangeMode === 'last') {
      // LAST = anchored 60-min window [wndStart, wndEnd]
      // Try windowed fetch first; if Ubidots returns empty, fallback to last-N + filter.
      await Promise.all(SENSORS.map(async s => {
        if (!s.address || !s.chart) return;

        let rows = await fetchVarWindow(
          deviceID, s.address, wndStart, wndEnd, /*hardCap*/ 4000
        );

        // Fallback: some accounts return empty sets for ?start/&end even when data exists
        if (!rows || rows.length === 0) {
          rows = await fetchVarLastN(
            deviceID,
            s.address,
            /*hardCap*/ 4000
          );
      ...
      await Promise.all(SENSORS.map(async s => {
        ...
        // Fetch newest N values then filter locally into the window.
    ...
    (function(){
      ...
      for (const s of SENSORS){
        ...
        const items = adjRdings2.map(r => {
          let v = r.v;
          if (typeof s.calibration === 'number') v += s.calibration;
          return { ts: Math.floor(r.ts/60000)*60000, v: Number.isFinite(v) ? v : null };
        }).filter(o => o.v != null);
...
async function updateBreadcrumbs(deviceID, rangeMinutes){
 ...
 const GAP_SPLIT_MS     = 2 * 60 * 1000;    // new segment if gap > 2 min
  ...
  __crumbAbort = (function(){
    ...
    catch(_) { return { abort(){}, get signal(){ return undefined; } }; }
  })();
...
function smoothSegment(arr){
  ...
  function ensureDwellStyles(){
    ...
    st.textContent = `
      ...
      .dwell-badge{background:rgba(255,255,255,0.90);border:1px solid #999;border-radius:3px;padding:2px 4px;
  ...
  function ensurePolylineDecorator(){
    ...
    return new Promise((resolve, reject)=>{
      ...
      s.src = 'https://unpkg.com/leaflet-polyline-decorator@1.9.3/dist/leaflet.polyline-decorator.min.js';
      s.async = true;
      s.onload = ()=> resolve();
    ...
    const rows = (gpsRows||[])
      ...
      .sort((a,b) => a.ts - b.ts);
        ...
        if (!rows.length) {
      console.warn('[breadcrumbs] No GPS rows for this window; leaving breadcrumbs unfixed so next refresh can retry.', {
        deviceID,
        selectedRangeMode,
        startTimeMs,
        endTimeMs
    ...
    for (let i=1; i<rows.length; i++){
      ...
      if (withinDwell(origin, cur)){
        ...
        if (dwellDurMs >= DWELL_TIME_MS){
          // Close the dwell; compute dwell centroid over [dwellOriginIdx .. lastIdx]
          let sumLat=0, sumLon=0, cnt=0;
          for (let k = dwellOriginIdx; k <= lastIdx; k++){ sumLat += seg[k].lat; sumLon += seg[k].lon; cnt++; }
          const center  = { lat: sumLat / cnt, lon: sumLon / cnt };
          const startPt = seg[dwellOriginIdx];
          const endPt   = lastBefore;

          const mover = seg.pop();   // remove 'cur' (first moving point)
          const prevSegIdx = Math.max(0, segments.length);
          pushSeg();                 // commits the just-finished (dwell-ended) segment
          seg = [ mover ];           // start next segment with the first moving point
          dwellOriginIdx = 0;
    ...
    segments.forEach((s, idx) => {
      ...
      if (kept.length >= 2) { usable.push(kept); origIndexOfUsable.push(idx); }
    });
...
await fetchSensorMapMapping();   // load aliases *after* device list, ensures fresh __aliases
  ...
  try {
    ...
    opt.text = (function(){var _n=Math.floor(Date.now()/1000),_hb=bestTs?Math.floor(bestTs/1000):0,_ls=(window.__deviceMap&&window.__deviceMap[label]&&window.__deviceMap[label].last_seen)||(sensorMap[label]&&(window.__deviceMap&&window.__deviceMap[label]&&window.__deviceMap[label].last_seen)||(sensorMap[label]&&sensorMap[label].last_seen)||0,_sa=_hb>0?(_n-_hb):(_n-_ls);return (ageOk?"\U0001F7E2":"\u26AA\uFE0F")+" "+getDisplayName(label)+" "+(ageOk?"("+Math.max(0,Math.round(_sa/60))+"m)":(_sa>86400?"(>24h)":"("+Math.round(_sa/3600)+"h)"));}());
  } catch (e) {
