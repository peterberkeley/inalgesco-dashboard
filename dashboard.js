...
function drawLive(data, SENSORS) {
  ...
  // Move map marker if we have a recent point (ignore >30 min old)
...
async function poll(deviceID, SENSORS) {
  ...
  // Fallback: separate lat/lng variables
  if (lat == null || lon == null) {
    ...
    if (latArr[0]?.value != null && lngArr[0]?.value != null) {
      lat = parseFloat(latArr[0].value);
      lon = parseFloat(lngArr[0].value);
    }
  }

  // Newest timestamp across what we got
  const candidates = [
    gpsArr[0]?.timestamp,
    iccArr[0]?.timestamp,
    sigArr?.[0]?.timestamp,
...
async function updateAll() {
  ...
  if (deviceID) {
    ...
    // refresh live polling loop
    if (pollTimer) clearInterval(pollTimer);
    await poll(deviceID, SENSORS); // run once immediately
