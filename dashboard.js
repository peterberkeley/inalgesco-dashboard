...
<body>
  ...
  <script>
    ...
    function fmt(v, p=1) {
      return (v == null || isNaN(v)) ? '–' : (+v).toFixed(p);
    }
    function isoHHMM(ts) {
    ...
    async function poll() {
      ...
      const [gpsA, sigA, voltA, spdA, n1A, n2A, n3A] = await Promise.all(
        Object.values(feeds).map(k => fetchFeed(k))
      );
      ...
      [
        ...
        if (live.ts !== lastTs[id]) {
          ...
          lastTs[id] = live.ts;
          if (s.chart.data.labels.length > CONFIG.historyLength) {
    ...
    function drawLive(d) {
      document.getElementById('latest').innerHTML = [
        ['Local Time',   new Date(d.ts).toLocaleString()],
        ['Lat',          fmt(d.lat, 6)],
        ['Lon',          fmt(d.lon, 6)],
        ['Alt (m)',      fmt(d.alt, 1)],
        ['Sats',         fmt(d.sats, 0)],
        ['Speed (km/h)', fmt(d.speed, 1)],
        ['RSSI (dBm)',   fmt(d.signal, 0)],
        ['Volt (mV)',    fmt(d.volt, 2)],
        ['NR1 °F',       fmt(d.nr1, 1)],
        ['NR2 °F',       fmt(d.nr2, 1)],
        ['NR3 °F',       fmt(d.nr3, 1)]
    ...
    async function onExport() {
      ...
      expStatus.textContent = 'Download ready.';
      exporting = false;
