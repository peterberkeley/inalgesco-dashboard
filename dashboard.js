...
(() => {
  ...
  // [1a] STATIC DEVICE LIST (skycafe-1 to skycafe-24)
  ...
  async function updateCharts() {
    ...
    await Promise.all(SENSORS.map(async s => {
      ...
      rows.reverse();
      const objs = rows.map(r => ({ x: formatTime12h(r.created_at), y: parseFloat(r.value) || null, fullTime: new Date(r.created_at).toLocaleString() }));
      s.chart.data.labels = objs.map(o => o.x);
      s.chart.data.datasets[0].data = objs.map(o => o.y);
      s.chart.update();
    }));
  }
  ...
  async function poll() {
    ...
    const [gpsA, sigA, voltA, spA, n1A, n2A, n3A, icA] = await Promise.all([
      ...
      fetchFeed(feeds.nr2),
      fetchFeed(feeds.nr3),
      fetchFeed(feeds.iccid)
    ]);
    let lat=0, lon=0;
    try { const g = JSON.parse(gpsA[0]?.value); lat = g.lat; lon = g.lon; } catch {}
    const pick = arr => parseFloat(arr[0]?.value) || null;
